...
class WC_Form_Handler {
	...
	public function order_again() {
		...
		foreach ( $order->get_items() as $item ) {
			...
			if ( ! apply_filters( 'woocommerce_add_to_cart_validation', true, $product_id, $quantity, $adminfee, $variation_id, $variations, $cart_item_data ) ) {
				continue;
			}

			WC()->cart->add_to_cart( $product_id, $quantity, $adminfee, $variation_id, $variations, $cart_item_data );
		}
	...
	public function add_to_cart_action( $url = false ) {
		...
		if ( 'variable' === $add_to_cart_handler ) {
			...
			if ( $all_variations_set ) {
				// Add to cart validation
				$passed_validation 	= apply_filters( 'woocommerce_add_to_cart_validation', true, $product_id, $quantity, $adminfee, $variation_id, $variations );

				if ( $passed_validation ) {
					if ( WC()->cart->add_to_cart( $product_id, $quantity, $adminfee, $variation_id, $variations ) ) {
