	...
	<?php foreach ( array_reverse( $subscriptions ) as $subscription_key => $subscription_details ) : ?>
		...
		<tr class="order">
			...
			<td class="subscription-title">
				...
				<?php $item_meta = new WC_Order_Item_Meta( $order_item['item_meta'] ); ?>
				<p>
				<?php $item_meta->display( true ); ?>
				<p>
			</td>
			...
			<td class="subscription-end">
				...
				<?php else : ?>
					<?php if ( in_array( $subscription_details['status'], array( 'cancelled', 'switched' ) ) ) : ?>
						<?php $end_of_prepaid_term = wp_next_scheduled( 'scheduled_subscription_end_of_prepaid_term', array( 'user_id' => (int)$user_id, 'subscription_key' => $subscription_key ) ); ?>
