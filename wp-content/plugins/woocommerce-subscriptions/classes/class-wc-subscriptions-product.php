...
class WC_Subscriptions_Product {
	...
	public static function user_can_not_delete_subscription( $allcaps, $caps, $args ) {
		...
		if ( isset( $args[0] ) && in_array( $args[0], array( 'delete_post', 'delete_product' ) ) && isset( $args[2] ) ) {
			...
			if ( false !== $product && 'trash' == $product->post->post_status && $product->is_type( array( 'subscription', 'variable-subscription', 'subscription_variation' ) ) ) {
				...
				$subscription_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM `wp_woocommerce_order_itemmeta` WHERE `meta_key` = '_product_id' AND `meta_value` = %d", $product_id ) );
