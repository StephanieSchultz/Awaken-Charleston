...
class WC_Gateway_Authorize_Net_AIM extends SV_WC_Payment_Gateway_Direct {
	...
	public function log_api_communication( $request, $response ) {
		...
		$this->add_debug_message( sprintf( __( "Request Time (s): %s\nRequest Method: %s\nRequest URI: %s\nRequest Body: %s", WC_Authorize_Net_AIM::TEXT_DOMAIN ), $request['time'], $request['method'], $request['uri'], $request['body'] ), 'message', true );

		if ( $response ) {
			$this->add_debug_message( sprintf( __( "Response Code: %s\nResponse Body: %s", WC_Authorize_Net_AIM::TEXT_DOMAIN ), $response['code'], $response['body'] ), 'message', true );
		}
...
} // end \WC_Gateway_Authorize_Net_AIM class
