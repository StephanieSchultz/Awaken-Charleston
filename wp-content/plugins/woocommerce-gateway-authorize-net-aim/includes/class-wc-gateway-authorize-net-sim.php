	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			$authnet_request = array(
				...
				"x_tax"                => "Order Tax<|>Order Tax<|>" . SV_WC_Plugin_Compatibility::wc_format_decimal( $order->order_tax, 2 ),
	...
	public function validate_fields() {
		...
		if ( 'yes' == $this->cvv ){
			...
			if ( ! ctype_digit( $cardCSC ) ) {
				SV_WC_Plugin_Compatibility::wc_add_notice( __( 'Card security code is invalid (only digits are allowed)', WC_Authorize_Net_AIM::TEXT_DOMAIN ), 'error' );
				return false;
			}

			if ( ( strlen( $cardCSC ) != 3 && in_array( $cardType, array('Visa', 'MasterCard', 'Discover' ) ) ) || ( strlen( $cardCSC ) != 4 && $cardType == 'American Express' ) ) {
				SV_WC_Plugin_Compatibility::wc_add_notice( __( 'Card security code is invalid (wrong length)', WC_Authorize_Net_AIM::TEXT_DOMAIN ), 'error' );
				return false;
			}
		}
		...
		if ( ! ctype_digit( $cardExpirationMonth ) || ! ctype_digit( $cardExpirationYear ) ||
			...
			SV_WC_Plugin_Compatibility::wc_add_notice( __( 'Card expiration date is invalid', WC_Authorize_Net_AIM::TEXT_DOMAIN ), 'error' );
			return false;
		...
		if ( empty( $cardNumber ) || ! ctype_digit( $cardNumber ) ) {
			SV_WC_Plugin_Compatibility::wc_add_notice( __( 'Card number is invalid', WC_Authorize_Net_AIM::TEXT_DOMAIN ), 'error' );
			return false;
		}
